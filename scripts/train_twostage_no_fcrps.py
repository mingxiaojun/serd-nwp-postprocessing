...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Train Table 2 two-stage residual diffusion without fCRPS"
    )
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs/checkpoints/twostage_no_fcrps")
    parser.add_argument(
        "--data_root_glob",
        type=str,
    ...
    parser.add_argument("--lambda_fcrps", type=float, default=0.0)
...
def show_samples(x, title="Samples", save_dir="./checkpoints", epoch=None, vmin=270, vmax=290):
    ...
    if epoch is not None:
        filename = os.path.join(save_dir, f"{title.replace(' ', '_')}_epoch_{epoch}_twostage_no_fcrps.png")
    else:
        filename = os.path.join(save_dir, f"{title.replace(' ', '_')}_twostage_no_fcrps.png")
    ...
    try:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for fc, err, label, valid_time, init_time in tqdm(
                ...
                if args.lambda_fcrps > 0:
                    loss_fcrps = ensemble_fcrps(ens, y.float())
                else:
                    loss_fcrps = torch.zeros((), device=device, dtype=loss_score.dtype)
            ...
            if is_main_process():
                latest_full_ckpt_path = os.path.join(args.save_dir, "vesde_physcond_twostage_no_fcrps_latest_full.pth")
            ...
            if (epoch + 1) % args.eval_every == 0:
                ...
                if is_main_process():
                    ...
                    if (epoch + 1) % args.save_every == 0:
                        ckpt_path = os.path.join(args.save_dir, f"vesde_physcond_twostage_no_fcrps_epoch_{epoch + 1}.pth")
                        save_training_checkpoint(
                            ckpt_path,
                            model,
                            optimizer,
                    ...
                    if mse_norm < best_metric:
                        ...
                        best_path = os.path.join(args.save_dir, "vesde_physcond_twostage_no_fcrps_best.pth")
                        save_training_checkpoint(
                            best_path,
                            model,
                            optimizer,
        ...
        if is_main_process():
            np.save(
                os.path.join(args.save_dir, "all_train_losses_twostage_no_fcrps.npy"),
                np.array(all_train_losses if all_train_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_train_score_losses_twostage_no_fcrps.npy"),
                np.array(all_train_score_losses if all_train_score_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_train_fcrps_losses_twostage_no_fcrps.npy"),
                np.array(all_train_fcrps_losses if all_train_fcrps_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_valid_mses_phys_twostage_no_fcrps.npy"),
                np.array(all_eval_phys if all_eval_phys is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_valid_mses_norm_twostage_no_fcrps.npy"),
                np.array(all_eval_norm if all_eval_norm is not None else []),
            )

            final_path = os.path.join(args.save_dir, "vesde_physcond_twostage_no_fcrps_final.pth")
