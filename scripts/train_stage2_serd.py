...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Train SERD stage-2 conditional VE-SDE diffusion with score loss and fCRPS"
    )
...
def show_samples(x, title="Samples", save_dir="./checkpoints", epoch=None, vmin=270, vmax=290):
    ...
    if epoch is not None:
        filename = os.path.join(save_dir, f"{title.replace(' ', '_')}_epoch_{epoch}_serd_v1_stage2_vesde_fcrps.png")
    else:
        filename = os.path.join(save_dir, f"{title.replace(' ', '_')}_serd_v1_stage2_vesde_fcrps.png")
...
def main():
    ...
    try:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if is_main_process():
                latest_full_ckpt_path = os.path.join(args.save_dir, "vesde_physcond_serd_v1_stage2_fcrps_latest_full.pth")
            ...
            if (epoch + 1) % args.eval_every == 0:
                ...
                if is_main_process():
                    ...
                    if (epoch + 1) % args.save_every == 0:
                        ckpt_path = os.path.join(args.save_dir, f"vesde_physcond_serd_v1_stage2_fcrps_epoch_{epoch + 1}.pth")
                        save_training_checkpoint(
                            ckpt_path,
                            model,
                            optimizer,
                    ...
                    if mse_norm < best_metric:
                        ...
                        best_path = os.path.join(args.save_dir, "vesde_physcond_serd_v1_stage2_fcrps_best.pth")
                        save_training_checkpoint(
                            best_path,
                            model,
                            optimizer,
        ...
        if is_main_process():
            np.save(
                os.path.join(args.save_dir, "all_train_losses_serd_v1_stage2_fcrps.npy"),
                np.array(all_train_losses if all_train_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_train_score_losses_serd_v1_stage2_fcrps.npy"),
                np.array(all_train_score_losses if all_train_score_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_train_fcrps_losses_serd_v1_stage2_fcrps.npy"),
                np.array(all_train_fcrps_losses if all_train_fcrps_losses is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_valid_mses_phys_serd_v1_stage2_fcrps.npy"),
                np.array(all_eval_phys if all_eval_phys is not None else []),
            )
            np.save(
                os.path.join(args.save_dir, "all_valid_mses_norm_serd_v1_stage2_fcrps.npy"),
                np.array(all_eval_norm if all_eval_norm is not None else []),
            )

            final_path = os.path.join(args.save_dir, "vesde_physcond_serd_v1_stage2_fcrps_final.pth")
